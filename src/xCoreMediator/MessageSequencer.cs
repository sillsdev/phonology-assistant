	...
	public sealed class MessageSequencer : IFWDisposable
	{
#if TESTMS
		private int m_obj = 0;
		private bool m_matchingHvo = false;
#endif
		private Control m_master;
		private IReceiveSequentialMessages m_receiver;
		private SafeQueue m_messages = new SafeQueue();
		private bool m_fReentering = false;
		private int m_cLevelsOfSequentialBlockInMessageHandler = 0; // See Begin/EndSequentialBlock.
		...
		public void SequenceWndProc(ref Message m)
		{
			...
			if (m_master.GetType().Name == "AtomicReferenceView" && m_obj == 0)
			{
				...
				m_matchingHvo = m_obj == 6166; // (m_obj == 6166 || m_obj == 6792);
			}
#endif
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc start: " + m_obj.ToString());
#endif
			CheckDisposed();
#if TESTMS
			...
			{
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc: MethodNeedsSequencing=false: normal handling of message: " + m.ToString());
#endif
			...
			{
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc: m_fReentering==true: cache  message: " + m.ToString());
#endif
			...
			{
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc m_fReentering==false");
#endif
				try
				{
#if TESTMS
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc try: reset m_fReentering to true; original value: " + m_fReentering.ToString());
#endif
					m_fReentering = true;
#if TESTMS
					//Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc try: call m_receiver.OriginalWndProc(ref m)");
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc normal handling of watched message start: " + m.ToString());
#endif
					m_receiver.OriginalWndProc(ref m);
#if TESTMS
					//Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc try: finished call m_receiver.OriginalWndProc(ref m)");
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc normal handling of watched message end: " + m.ToString());
#endif
...
#if TESTMS
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc try: call DoPendingMessages()");
#endif
					...
					DoPendingMessages();
#if TESTMS
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc try: finished call DoPendingMessages()");
#endif
				}
				finally
				{
#if TESTMS
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc finally: reset m_fReentering to false; original value: " + m_fReentering.ToString());
#endif
					m_fReentering = false;
				}
			}
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceWndProc end: " + m_obj.ToString());
#endif
		}
		...
		{
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.DoPendingMessages start: " + m_obj.ToString());
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.DoPendingMessages Asserting m_fReentering to be true; " + "original value: " + m_fReentering.ToString());
			if (!m_fReentering)
			{
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.DoPendingMessages Failed Assert: Debug.Assert(m_fReentering);");
...
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.DoPendingMessages Passed Assert: Debug.Assert(m_fReentering);");
#endif
			while (m_messages.Count > 0)
			{
				Message m1 = (Message)m_messages.Remove();
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.DoPendingMessages handle cached message (start): " + m1.ToString());
#endif
				m_receiver.OriginalWndProc(ref m1);
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.DoPendingMessages resend message (done): " + m1.ToString());
#endif
			}
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.DoPendingMessages end: " + m_obj.ToString());
#endif
		}
		...
		{
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint start: " + m_obj.ToString());
#endif
			CheckDisposed();
#if TESTMS
...
#endif
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint check m_fReentering value; original value: " + m_fReentering.ToString());
#endif

			if (m_fReentering)
			{
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint: m_fReentering==true: call m_master.Invalidate()");
#endif
				m_master.Invalidate();
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint finished m_master.Invalidate()");
#endif
			}
			else
			{
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint try: reset m_fReentering to true; original value: " + m_fReentering.ToString());
#endif
				m_fReentering = true;
				try
				{
#if TESTMS
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint try:  call m_receiver.OriginalOnPaint start: " + e.ToString());
#endif
					m_receiver.OriginalOnPaint(e);
#if TESTMS
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint try:  call m_receiver.OriginalOnPaint end: " + e.ToString());
#endif
				}
				finally
				{
#if TESTMS
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint finally: call DoPendingMessages()");
#endif
					DoPendingMessages();
#if TESTMS
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint try: finished call DoPendingMessages()");
					Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint finally: reset m_fReentering to false; original value: " + m_fReentering.ToString());
#endif
					m_fReentering = false;
				}
			}
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.SequenceOnPaint end: " + m_obj.ToString());
#endif
		}
		...
		{
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.PauseMessageQueueing start: " + m_obj.ToString());
#endif
			CheckDisposed();
#if TESTMS
			//Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.PauseMessageQueueing Must not be disposed.");
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.PauseMessageQueueing check m_fReentering vakue; original value: " + m_fReentering.ToString());
#endif

			if (m_fReentering)
			{
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.PauseMessageQueueing: m_fReentering==true: call DoPendingMessages()");
#endif
				DoPendingMessages();
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.PauseMessageQueueing: finished call DoPendingMessages()");
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.PauseMessageQueueing: reset m_fReentering to false; original value: " + m_fReentering.ToString());
#endif
				m_fReentering = false;
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.PauseMessageQueueing end returning true: " + m_obj.ToString());
#endif
				return true;
			}
			else
			{
#if TESTMS
				Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.PauseMessageQueueing end returning false: " + m_obj.ToString());
#endif
				return false;
			}
		}
		...
		{
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.ResumeMessageQueueing start: " + m_obj.ToString());
#endif
			CheckDisposed();
#if TESTMS
			//Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.ResumeMessageQueueing Must not be disposed.");
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.PauseMessageQueueing: reset m_fReentering to true; original value: " + m_fReentering.ToString());
#endif

			m_fReentering = true;
#if TESTMS
			Debug.WriteLineIf(m_matchingHvo, "MessageSequencer.ResumeMessageQueueing end: " + m_obj.ToString());
#endif
		}
