...
namespace XCore
{
	...
	public class CommandChoice : ChoiceBase
	{
		...
		{
			return QueryDisplayProperties(m_mediator, CommandObject, m_defaultVisible, Label);
		}
		...
		static public UIItemDisplayProperties QueryDisplayProperties(Mediator mediator, Command command, bool defaultVisible, string label)
		{
			...
			UIItemDisplayProperties display =new UIItemDisplayProperties(label, hasReceiver, command.IconName, defaultVisible);
	...
	public class BoolPropertyChoice : ChoiceBase
	{
		...
		public bool BoolPropertyValue
		{
			get
			{
				return 	m_mediator.PropertyTable.GetBoolProperty(BoolPropertyName, DefaultBoolPropertyValue);
			}
		}
		...
		public bool Checked
		{
			get
			{
				...
				return m_mediator.PropertyTable.GetBoolProperty(BoolPropertyName, DefaultBoolPropertyValue)  == true;
			}
		}
		...
		override public void OnClick(object sender, System.EventArgs args)
		{
			...
			//toggle our value
			m_mediator.PropertyTable.SetProperty(BoolPropertyName, !this.BoolPropertyValue);
		}
...
//		}
		...
		public bool Checked
		{
			get
			{
				Object x = m_mediator.PropertyTable.GetValue(PropertyName);
				return x!=null && (string)x== PropertyValueSetting;
			}
		}
		#endregion Properties
		...
		override public void OnClick(object sender, System.EventArgs args)
		{
			...
			{
				m_mediator.PropertyTable.SetProperty(this.PropertyName+"Parameters", ParameterNode);
				...
				m_mediator.PropertyTable.SetPropertyPersistence(this.PropertyName+"Parameters", false);
			}
			
			//remember, each of these calls to SetProperty() generate a broadcast, so the order of these two calls
			//is relevant.
			m_mediator.PropertyTable.SetProperty(this.PropertyName, Value);

			if(ParameterNode !=null)
			{
				//since we cannot persist the parameters, it's safer to not persist the choice either.
				m_mediator.PropertyTable.SetPropertyPersistence(this.PropertyName, false);
			}
		}
		...
		override public UIItemDisplayProperties GetDisplayProperties()
		{
			UIItemDisplayProperties display =new UIItemDisplayProperties(this.Label, true, ImageName, this.m_defaultVisible);
			display.Checked = this.Checked;
