	...
	public class RecordCache : List<RecordCacheEntry>, IDisposable
	{
		...
		public void Save(string filename)
		{
			try
			{
				...
				foreach (PaFieldInfo fieldInfo in App.Project.FieldInfo.Where(fi => fi.IsCustom))
		...
		private void FindAutoGeneratedAmbigousSequences()
		{
			...
			var phoneticField = App.Fields.Single(f => f.Type == FieldType.Phonetic).Name;
			
			foreach (var entry in this)
			{
				var phonetic = entry.GetValue(phoneticField);
				if (phonetic != null)
				{
					var seqs = App.IPASymbolCache.FindAmbiguousSequences(phonetic);
					if (seqs != null)
		...
		{
			var phoneticField = App.Fields.Single(f => f.Type == FieldType.Phonetic);

			if (phoneticField != null)
				TempRecordCache.Add(entry.Id, entry[phoneticField.Name]);
