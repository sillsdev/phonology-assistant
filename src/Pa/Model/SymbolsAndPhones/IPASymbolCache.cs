...
namespace SIL.Pa.Model
{
	...
	public class IPASymbolCache : Dictionary<int, IPASymbol>
	{
		...
		//    {
		//        foreach (var ambiguousSeq in m_unsortedAmbiguousSeqList)
		...
		//    {
		//        foreach (var info in m_toneLetters.Values.Where(info =>
		...
		//    phonetic = m_project.TranscriptionChanges.Convert(phonetic);
		...
		//    var phones = PhoneticParser(phonetic, normalize, out uncertainPhones);
		...
		//                if (LogUndefinedCharactersWhenParsing && UndefinedCharacters != null)
		//                    UndefinedCharacters.Add(c, origPhoneticRunBeingParsed);
