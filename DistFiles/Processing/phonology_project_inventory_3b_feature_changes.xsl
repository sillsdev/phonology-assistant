<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- phonology_project_inventory_3b_feature_changes.xsl 2010-04-09 -->
		...
		<articulatoryFeatureChanges>
			<add>
				<xsl:for-each select="$articulatoryFeatures/feature">
					...
					<xsl:choose>
						<xsl:when test="$sequence/symbol/articulatoryFeatures/feature[. = $feature]" />
						<xsl:otherwise>
							<xsl:copy-of select="." />
						</xsl:otherwise>
					</xsl:choose>
			...
			<remove>
				<xsl:for-each select="$sequence/symbol/articulatoryFeatures/feature">
					<xsl:variable name="feature" select="." />
					<xsl:choose>
		...
		<binaryFeatureChanges>
			<add>
				<xsl:for-each select="$binaryFeatures/feature">
					<xsl:variable name="feature" select="." />
					<xsl:choose>
						<xsl:when test="$sequence/symbol/binaryFeatures/feature[. = $feature]" />
						<xsl:otherwise>
							<xsl:copy-of select="." />
						</xsl:otherwise>
					</xsl:choose>
			...
			<remove>
				<xsl:for-each select="$sequence/symbol/binaryFeatures/feature">
					<xsl:variable name="feature" select="." />
					<xsl:choose>
