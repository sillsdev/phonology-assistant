<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- phonology_project_inventory_6_similar_pairs.xsl 2012-05-23 -->
  ...
  <xsl:template match="@* | node()">
	...
	<xsl:template name="pair">
		...
		<xsl:variable name="articulatorDifferences">
			<xsl:call-template name="similarity">
				<xsl:with-param name="features1" select="$features1" />
				<xsl:with-param name="features2" select="$features2" />
				<xsl:with-param name="similarityMetric" select="$similarityMetric" />
				<xsl:with-param name="articulator" select="'true'" />
			</xsl:call-template>
		</xsl:variable>
		<xsl:variable name="articulatorLength" select="string-length($articulatorDifferences)" />
		<xsl:if test="$articulatorLength &lt;= 1">
			<xsl:variable name="ordinaryDifferences">
				<xsl:call-template name="similarity">
					<xsl:with-param name="features1" select="$features1" />
					<xsl:with-param name="features2" select="$features2" />
					<xsl:with-param name="similarityMetric" select="$similarityMetric" />
					<xsl:with-param name="articulator" select="'false'" />
				</xsl:call-template>
			</xsl:variable>
			<xsl:variable name="ordinaryLength" select="string-length($ordinaryDifferences)" />
			<xsl:variable name="differences" select="$articulatorLength + $ordinaryLength" />
	...
	<xsl:template name="similarity">
		<xsl:param name="features1" />
		<xsl:param name="features2" />
		<xsl:param name="similarityMetric" />
		<xsl:param name="articulator" />
		<xsl:for-each select="$similarityMetric/featureMetric[@articulator = $articulator]">
