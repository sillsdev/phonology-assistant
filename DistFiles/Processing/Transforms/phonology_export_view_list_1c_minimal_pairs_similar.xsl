...
>
  ...
  <!-- phonology_export_view_list_1c_minimal_pairs_similar.xsl 2011-11-04 -->
  ...
  <!-- classify the list of minimal pairs as more-similar, less-similar, least-similar. -->
	...
	<xsl:template match="xhtml:table[contains(@class, 'list')][xhtml:tbody[contains(@class, 'group')]/xhtml:tr[@class = 'heading']/xhtml:th[@class = 'Phonetic pair']]">
		...
		<xsl:variable name="searchPattern" select="$details/xhtml:li[@class = 'search pattern']" />
		...
		<!-- A project phonetic inventory file contains more-similar and less-similar pairs of segments. -->
