<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- phonology_project_inventory_2b_place_or_backness.xsl 2011-08-22 -->
  <!-- Determine sort order by place of articulation or backness. -->

  <xsl:output method="xml" version="1.0" encoding="UTF-8" omit-xml-declaration="no" indent="no" />

  <xsl:variable name="chartClass1" select="'colgroup'" />
  <xsl:variable name="chartClass2" select="'rowgroup'" />
  <xsl:variable name="sortKeyClass" select="'place_or_backness'" />
