<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- phonology_project_inventory_2b_base_sequences.xsl 2010-05-21 -->
  <!-- Merge articulatory features of base character sequences. -->
	...
	<!--   If it has a class="row" attribute, add a position="1" or position="2" attribute to the corresponding feature of its subclass. -->
  ...
  <xsl:template match="@* | node()">
	...
	<xsl:template match="symbol[not(isBase/@primary = 'false')]/articulatoryFeatures/feature" mode="mergeFeatures">
		...
		<xsl:if test="not($symbolOfPattern/remove/feature[@subclass = $subclass])">
			<xsl:copy>
				...
				<xsl:if test="$pattern/symbol[@primary = 'false']/retain/feature[@subclass = $subclass][@class = 'row'] and ../../../symbol[isBase/@primary = 'false']/articulatoryFeatures/feature[@subclass = $subclass][. != $feature]">
					<!-- If any non-primary symbol of the pattern retains (as a row feature) a different feature (with the same subclass), -->
					<!-- add a position attribute to distinguish this feature. For descriptions of diphthongs. -->
	...
	<xsl:template match="symbol[isBase/@primary = 'false']/articulatoryFeatures/feature" mode="mergeFeatures">
		...
		<xsl:if test="$featureOfPattern and not(../../../symbol[isBase/@primary = 'true']/articulatoryFeatures/feature[. = $feature])">
			<xsl:copy>
				<xsl:apply-templates select="@*">
					<xsl:with-param name="featureOfPattern" select="$featureOfPattern" />
				</xsl:apply-templates>
	...
	<xsl:template match="symbol[isBase/@primary = 'false']/articulatoryFeatures/feature/@class">
		<xsl:param name="featureOfPattern" />
		<xsl:attribute name="class">
			<xsl:choose>
				<xsl:when test="$featureOfPattern/@class">
					<xsl:value-of select="$featureOfPattern/@class" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="." />
				</xsl:otherwise>
